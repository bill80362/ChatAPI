...
class MsgStream
{
    ...
    public function __construct($UserID,$isStream=false,$_Token="")
    {
        ...
        //設定用戶連線中
        if($this->isStream)
            $this->redis->hset( 'online' , $this->Key , $_Token ) ;//上線標記
    }
    //SSE用戶關閉視窗會觸發
    public function __destruct()
    {
        //目前用戶的redis的token是否一致(不同代表同用戶重複登入)
        $redisToken = $this->redis->hget('online',$this->Key );
        //用戶離線
        if($this->isStream && $this->Token==$redisToken){
            $this->redis->hdel( 'online', $this->Key ) ; //bill離線
            $this->redis->del($this->Key);//清空對列
        }
        //測試用-離線會記錄在DB
...
//        $oTestSSE->create(array("title"=>"用戶:，離線","content"=>MEMCACHE_TEST));
    ...
    public function isUserStream(){
        //bill是否在現在
        if($this->redis->hexists( 'online' , $this->Key ))
            return true;
        else
            return false;
    }
    ...
    public function getDataNoLock(){
        $task = $this->redis->rpop($this->Key);
        $task = json_decode($task, true);
        return $task;
    }
    //新增資料
    public function pushData($_Data){
        $this->redis->lPush($this->Key, json_encode($_Data));
    }
}